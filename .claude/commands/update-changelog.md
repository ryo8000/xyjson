...
3. Run `git log --oneline` since the last release tag or the previous version's date to collect all commits since the last release.
4. Based on the commits, categorize changes into:
   - **Added**: new features (feat: commits)
   - **Changed**: updates, dependency bumps, config changes (chore:/bump commits)
